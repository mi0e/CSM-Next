...
test('Worker exposes public theme settings with environment fallbacks', async () => {
  ...
  assert.equal(data.settings.transparencyEnabled, false)
  assert.equal(data.settings.transparencyMode, 'soft')
  assert.equal(data.settings.panelOpacity, 1)
  assert.equal(data.settings.panelBlur, 18)
...
test('Worker requires a valid upstream login before writing theme settings', async () => {
  ...
  try {
    ...
    assert.equal(body.settings.transparencyEnabled, true)
    assert.equal(body.settings.transparencyMode, 'glass')
    assert.equal(body.settings.panelBlur, 18)
    ...
    assert.equal(loadedBody.settings.transparencyEnabled, true)
    assert.equal(loadedBody.settings.transparencyMode, 'glass')
