...
- 首页可见节点一次性载入过去 24 小时延迟并聚合为 24 个等宽色块,详情页展示 Ping、丢包、波动和悬浮数据
...
- 上游支持优化长历史接口时,首页可见节点按需读取一次过去 24 小时历史并聚合为 24 个延迟色块;后续仅由 WebSocket 更新末端时间块,不会定时重复查询 D1。旧版上游继续使用零历史请求的会话趋势。
