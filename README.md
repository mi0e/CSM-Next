...
- 新版上游开启「三网详情」后,首页直接使用 `/api/servers` 返回的 `servers[].ping` / `servers[].loss` 与 `sysConfig.latency_window`（当前默认最近 2 小时、20 个点）,不再逐节点请求 `/api/history/all`;后续 WebSocket 样本增量合并到末端时间块。旧版上游或未开启三网详情时使用零历史请求的会话趋势。
